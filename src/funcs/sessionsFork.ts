 ...
 */
export function sessionsFork(
  client: MixCore,
  request: operations.ForkSessionRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.ForkSessionResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ForkSessionRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/api/sessions/{id}/fork")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.ForkSessionResponse$inboundSchema),
