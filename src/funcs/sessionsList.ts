...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListSessionsRequest$outboundSchema.optional().parse(value),
  ...
  const query = encodeFormQuery({
    "includeSubagents": payload?.includeSubagents,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, z.array(models.SessionData$inboundSchema)),
