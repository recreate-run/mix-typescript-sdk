...
import { MixDefaultError } from "../models/errors/mixdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | MixDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | MixDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new MixDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new MixDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
